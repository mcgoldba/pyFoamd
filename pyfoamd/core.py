...
############################ Public Member Functions ############################
    ...
    def cloneSim(self):
        ...
        if (len(self.add_args) == 1):
            srcDir = self.of_case_dir
            destDir = os.path.join(rootDir, self.add_args[0])
        elif (len(self.add_args) == 0):
            srcDir = self.of_case_dir
            n = re.findall('[0-9]+', os.path.split(self.of_case_dir)[-1])[-1]
            m = int(n)+1
...
#                            copy=False
    ...
    def getLatestTime(self, directory):
    ...
    def appendBlockEntry(self, value, blockList=None, lineNum=None, searchValues=False):
        if blockList != None:
            self.__appendBlockEntryWithBlockName(value, blockList, searchValues=searchValues)
        elif lineNum != None:
            self.__appendBlockEntryWithLineNum(value, lineNum, searchValues=searchValues)
        else:
    ...
    def __appendBlockEntryWithBlockName(
        ...
        _, stop = self.__ofDictFindBlockEntryStartStop(
    ...
    def replaceEntry(self, ofValue, rType, silent=True):
        ...
        if rType == 'string':
            found = self.__replaceStringEntry(
                ofValue.name,
                ofValue.value,
                file
            )
        elif rType == 'singleLine':
            found = self.__replaceSingleLineEntry(
                ofValue.name,
                ofValue.value,
                ofValue.asString(),
                file
    ...
    def __findBlockEntryStartStop(self, file, blockList, searchValues=False):
        #TODO:  Account for the case of opening anf closing parenthesis or barackets
        #  on the same line.
        relPath = file
        file = os.path.join(self.of_case_dir, file)
...
#                                    print("Warning:  No values found matching block '"+block+"' within "+file)
    ...
    def __removeEntry(self, file, blockList, searchValues=False):
        ...
        try:
            start, stop = self.__ofDictFindBlockEntryStartStop(
                file,
                blockList,
                searchValues=searchValues
            )
    ...
    def removeBlockEntries(self, file, blockList, searchValues=False):
    ...
    def replaceStringEntry(self, key, val, file, silent=False):
    ...
    def replaceSingleLineEntry(self, key, val, string, file, silent=False):
        ...
        with open(file+"_old", 'r') as old, open(file, 'w') as new:
            ...
            for line in old:
                ...
                if (len(line.split()) > 0 and
                    line.split()[0] == key):
                    #- Ensure that the found key is a single lineed value
                    if line.rstrip()[-1] != ';':
                        new.write(line)
                        continue
                    ...
                    for _ in range(nSpaces):
                        pre += ' '
                    new.write(pre+string)
                    found=True
                    if not silent:
                        relPath = file[len(self.of_case_dir):]
                        print("'"+str(key)+"' changed to '"+str(val)+"' in file:  "+relPath)
    ...
    def readOFDictFile(self, file):
        ...
        def argSwitcher(self, func, line, ofType):
            switcher = {
                self.__getOFDictReadLineType: len(line.split(" ")),
                self.__storeOFDictValue: (ofType,
                                          line[1:].remove('{()}').split(" ")
                                         )
            }
        ...
        with open(file, 'r') as f:
            for line in f:
                ...
                if lineStatus == 'endMultiLine':
                    ...
                    [status, lineStatus] = self.__getOFDictMultiLinePreviousType(line)
                lineStatus = 'read'
                count = 0
                while lineStatus != 'end':
                    ...
                    func = self.functionSwitcher(status)
                    if func is not None:
                        [status, lineStatus] = func(self.argSwitcher(func, line, ofType))



    def __getOFDictCommentLineType(self, startsWith):
        switcher = {
            '*/': ['commentedBlockEnd', 'end']
        }
        return [switcher.get(startsWith, 'commentedBlock'), 'end']

    def __getOFDictValueType(self, startsWith):
        switcher = {
            '(': 'list',
            '{': 'dict'
        }
    ...
    def __getOFDictMultiLineEntryType(self, line):
        switcher = {
            '(': 'list',
            '{': 'dict',
    ...
    def __ofMultiLineEntryReadNextLine(self, line):
        pass

    def __getOFDictReadLineType(self, length):
        switcher = {
            0: ['empty', 'end'],
    ...
    def __storeOFDictValue(self, line):
        pass

    def __appendOFDictValue(self, line):
        pass

    def __storeOFNamedListValue(self, line):
        pass

    def __getOFDictMultiLinePreviousType(self, line):
        switcher = {
            '{': 'dictName',
            '(': 'listName'
        }
    ...
    def __storeOFDictSingleLineSingleValuedEntry(self, line):
        values = line.split(" ")
        if len(values) != 2:
